...
（我想把最完美的 D2Admin 一次性呈现给大家，但是最好还是一步一步来）
...
创建页面最重要的 d2-container 组件进行了重构，详见 [页面容器](http://d2admin.fairyever.com/zh/components/container.html)，依旧是支持 `full` + `card` + `ghost` 三种模式，不同的是三种模式现在都分别支持 `header` 和 `footer` 插槽，并且都支持开关自定义滚动条。
